...
async def settings_cb(client, CallbackQuery, _):
    try:
        ...
        pass
    buttons = setting_markup(_)
    return await CallbackQuery.edit_message_text(_["setting_1"].format(CallbackQuery.message.chat.title, CallbackQuery.message.chat.id), reply_markup=InlineKeyboardMarkup(buttons))
...
async def settings_back_markup(client, CallbackQuery: CallbackQuery, _):
    ...
    if CallbackQuery.message.chat.type == "private":
        ...
        return await CallbackQuery.edit_message_text(_["start_2"].format(MUSIC_BOT_NAME), reply_markup=InlineKeyboardMarkup(buttons))
...
@app.on_callback_query(filters.regex(pattern="^(SEARCHANSWER|PLAYMODEANSWER|PLAYTYPEANSWER|AUTHANSWER|CMANSWER|COMMANDANSWER|SUGGANSWER|CM|AQ|VQ|PM|AU)$") & ~BANNED_USERS)
@languageCB
async def without_Admin_rights(client, CallbackQuery, _):
    ...
    if command == "AU":
        try:
            ...
            pass
        is_non_admin = await is_nonadmin_chat(CallbackQuery.message.chat.id)
        buttons = auth_users_markup(
            _) if is_non_admin else auth_users_markup(_, True)
    try:
        return await CallbackQuery.edit_message_reply_markup(reply_markup=InlineKeyboardMarkup(buttons))
...
@app.on_callback_query((filters.regex(pattern="^(LQA|MQA|HQA|LQV|MQV|HQV)$") & ~BANNED_USERS))
@ActualAdminCB
...
async def authusers_mar(client, CallbackQuery, _):
    ...
    if command == "AUTHLIST":
        ...
        if not _authusers:
            ...
            upl = InlineKeyboardMarkup([[InlineKeyboardButton(text=_["BACK_BUTTON"], callback_data="AU"), InlineKeyboardButton(
...
@app.on_callback_query((filters.regex(pattern="^(CLEANMODE|COMMANDELMODE|SUGGESTIONCHANGE)$") & ~BANNED_USERS))
@ActualAdminCB
