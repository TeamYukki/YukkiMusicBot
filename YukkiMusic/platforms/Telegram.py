...
class TeleAPI:
    ...
    async def get_filepath(self, audio: Union[bool, str] = None, video: Union[bool, str] = None):
        if audio:
            try:
                file_name = f"{audio.file_unique_id}." + \
                    ("ogg" if isinstance(audio, Voice)
        ...
        if video:
            try:
                file_name = f"{video.file_unique_id}." + \
        ...
        async def down_load():
            async def progress(current, total):
                ...
                upl = InlineKeyboardMarkup([[InlineKeyboardButton(
                    text="🚦 Cancel Downloading", callback_data="stop_downloading")]])
